...
const sizeClasses: Record<NonNullable<SheetContentProps["size"]>, string> = {
  sm: "max-w-sm",
  md: "max-w-md",
  lg: "max-w-lg",
};
  ...
  ({ className, size = "md", hideClose, children, ...props }, ref) => (
    <DialogPrimitive.Portal>
      ...
      <DialogPrimitive.Content
        ref={ref}
        className={cn(
          "fixed inset-y-0 right-0 z-50 flex w-full flex-col animate-slide-in-right bg-white shadow-floating ring-1 ring-slate-900/5 focus:outline-none dark:bg-slate-900 dark:ring-white/10",
          sizeClasses[size],
          className,
        )}
        {...props}
